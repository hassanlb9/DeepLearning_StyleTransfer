...
desired_size = 512 if torch.cuda.is_available() else 256  # use small size if no gpu

print(desired_size)
# scale imported image
# transform it into a torch tensor
loader = transforms.Compose([transforms.Resize(desired_size),  transforms.ToTensor()])  
...
style_img = image_loader(image_directory + "style7.jpg")
content_img = image_loader(image_directory + "content5.jpg")
...
def displayimg(tensor, title=None):
...
plt.figure()
displayimg(style_img, title='Style Image')

plt.figure()
displayimg(content_img, title='Content Image')
...
plt.figure()
displayimg(input_img, title='Input Image')
...
plt.figure()
displayimg(output, title='Output Image')
